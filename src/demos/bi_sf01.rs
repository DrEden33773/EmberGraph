...
async fn exec<S: AdvancedStorageAdapter + 'static>(plan_filename: &str) -> io::Result<()> {
...
pub async fn bi_1_on_sf_01() -> io::Result<()> {
  ...
  exec::<Neo4jStorageAdapter>("ldbc-bi-1.json").await
}
...
pub async fn bi_10_on_sf_01() -> io::Result<()> {
  ...
  exec::<SqliteStorageAdapter>("ldbc-bi-10.json").await
}

/// ☑️
pub async fn bi_11_on_sf_01() -> io::Result<()> {
  println!("Querying 'BI-11' on 'SF0.1' ...\n");
  exec::<Neo4jStorageAdapter>("ldbc-bi-11.json").await
}
